        ...
        try:
            if action == 'reset':
                ...
                if button == 'Buy Now':
                    ...
                    assert self.session['page_num'] < math.ceil(
                        self.session['max_products'] / 10)  # Seemed already capped at 50
                    ...
                    if self.session['page_type'] == 'search':
                        ...
                        assert button in self.session['option_types'], (
                        button, self.session['option_types'])  # must be options
